...
namespace Core.State.Components
{
  public class CoreStateViewModel : IDisposable,
    ICoreStateComponent
  {
    static readonly ConcurrentDictionary<string, int> s_InstanceCounts = new ConcurrentDictionary<string, int>();

    public CoreStateViewModel(IMediator mediator, IStore store, Subscriptions subs)
    {
